...
//-------------D114----------------------//
	...
	while (count2<L1_MAX_ENTRIES)
	{

	if(pcb->pagetable[count2]!=0)
	{
	
	printf("Process ID is : %lu,\t base address of the virtual page : %p\t Freeing physical page (page_number): %d\n",findpid1(pcb),&pcb->pagetable[count2],count2);
	
		
    	MemoryFreePte (pcb->pagetable[count2]);
	}
	...
	pcb->npages=0; // update the value in the PCB
...
//-------------------------------------------------D-114----------------------------//
  pcb->npages = 4;
	...
	{	
		...
		newly_assigned_page= MemoryAllocPage();                  // 3 pages for the text and data
	
		if(newly_assigned_page==0)
	{
		printf("Error occured: New page cannot be allocated for this process \n");
		
		exitsim();
	}
	pcb->pagetable[count1]=MemorySetupPte(newly_assigned_page);
	
	count1++;	
	
	}

// This is for the user stack
  
	newly_assigned_page = MemoryAllocPage ();
	...
	pcb->pagetable[L1_MAX_ENTRIES-1] = MemorySetupPte (newly_assigned_page);// user stack at the high end of the virtual address

	// the below code is for the stack area
...
//-----------------------------------------------------D-114--------------------------------//
	...
	pcb->sysStackArea = newly_assigned_page * MEMORY_PAGE_SIZE;
...
//-------------------D-114------------------//
 	...
 	QueueRemove(&pcb->l);	  
  	...
  	ProcessFreeResources(pcb); // will free the process's memory by calling the method QueueInsertLast fuction		
...
page_number= faultaddress / MEMORY_PAGE_SIZE; // calculate the page number for the address for which the page fault occurs

//first we need to check whether the process has alredy used the maximum number of pages it is allowed to occupy
...
if(currentPCB->npages==16)
{
	...
	printf("Memory full for this particular process\n");
	...
	return ProcessKill(currentPCB);
						...
						//else we have to assign  a new page for this default address

	newly_assigned_page=MemoryAllocPage();
	


						//Now check whether the new page is correctly assigned, if not we have to kill the process
if (newly_assigned_page==0)

{

	printf("Error occured: page could not assigned");
	return ProcessKill(currentPCB);

}

currentPCB->pagetable[page_number]= MemorySetupPte(newly_assigned_page); // the function retuns the Page table entry and stores in the page.


currentPCB->npages = currentPCB->npages + 1; // increment the number of pages for this process

printf("Process ID is  %lu,\t The Fault Address is : %p\t The Physical page number allocated: %d\n",findpid(currentPCB),faultaddress,page_number);
