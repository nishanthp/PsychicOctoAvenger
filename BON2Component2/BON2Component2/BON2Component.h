...
class Component
{
	...
	public:
		...
		void newMethod(std::string N, Model &model);
	#ifdef GME_ADDON
	...
	//------------------NishanthPrakash-----------------------//
		struct Node
		{
			std::string input1;					//Have initialized a structure for each Atom in the model
			std::string input2;
			std::string Name;					// Each structure contains one or two inputs and the Name represents the output
			std::string output;
		};

		
		std::vector <struct Node> array_Nodes;		// vector which holds all the structures
